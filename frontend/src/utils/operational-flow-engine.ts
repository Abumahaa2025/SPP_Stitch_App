...
/** Kowil received Ejar official notice — owner approval before fan-out. */
export async function onEjarNoticeReceived(input: {
  eventId: string;
  contractNumber: string;
  unit?: string;
  ...
  const contract = input.contractNumber || '—';
  const isExpiry = input.eventType?.includes('expir') || (typeof input.daysLeft === 'number' && input.daysLeft <= 30);
  await appendEvent({
    kind: 'notification_prepared',
    actor: 'employee',
    summaryKey: 'op.event.ejarReceived',
  ...
  await addPendingAction({
    kind: 'approve_ejar_notice',
    labelKey: isExpiry ? 'op.pending.ejarExpiry' : 'op.pending.ejarNotice',
    labelParams: { contract },
    payload: {
      eventId: input.eventId,
      contractNumber: contract,
