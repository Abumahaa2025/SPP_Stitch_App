...
| SPP architectural law covers SPP runtime only (`frontend/`, `backend/`) | A co-located Arabic product folder must not be mistaken for SPP | SPP documents do not govern `smart-employee/`; see Architecture Governance §6.3 |
...
*v1.2:* Scope correction — SPP Blueprint governs SPP only; co-located Arabic product is out of scope (Governance §6.3).
