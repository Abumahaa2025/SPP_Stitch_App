...
### Running the web app in a browser (Chrome gotcha)
...
Beta demo logins (via `POST /api/beta/login`): `demo.owner@spp.beta` / `SPP-Owner-26` (also `demo.tenant@spp.beta` / `SPP-Tenant-26`, `demo.tech@spp.beta` / `SPP-Tech-26`). The core AI feature is the "Smart property employee" chat at route `/brain` (`POST /api/chat`, body field is `text`).
